...
authorApp.post("/article", verifyToken("AUTHOR"), async (req, res) => {
  ...
  if (!author) {
    return res.status(404).json({ message: "Invalid author", error: "Invalid author" });
  }
  //cross check emails
  if (author.email != user.email) {
    return res.status(403).json({ message: "You are not authorized", error: "Not authorized to publish for this author" });
  }
...
authorApp.put("/articles", verifyToken("AUTHOR"), async (req, res) => {
  ...
  if (!modifiedArticle) {
    return res.status(403).json({ message: "Not authorized to edit article", error: "Not authorized or article not found" });
  }
  //send res
